...
class Blockchain:
    ...
    def __init__(self):
        ...
        self.transactions = []
        self.allchain = list(mycol.find({}, {'_id': 0}))
        self.chain = []
        self.nodes = set()
    ...
    def create_block(self, nonce, previous_hash):
        ...
        """
        allchain = list(mycol.find({}, {'_id': 0}))
        block = {'block_number': len(allchain) + 1,
                 'timestamp': time(),
    ...
    def hash(self, block):
        ...
        block_string = json_util.dumps(block, sort_keys=True).encode()
    ...
    def valid_proof(self, transactions, last_hash, nonce, difficulty=MINING_DIFFICULTY):
        ...
        # print("guess_hash[:difficulty] == '0' * difficulty")
    ...
    def valid_chain(self, chain):
        ...
        while current_index < len(chain):
            ...
            if block['previous_hash'] != self.hash(last_block):
                print("不等于上一个哈希"+(block['previous_hash'] != self.hash(last_block)).__str__())
                return False
    ...
    def resolve_conflicts(self):
        ...
        for node in neighbours:
            ...
            if response.status_code == 200:
                ...
                print("valid_chain"+self.valid_chain(chain).__str__())
    ...
    def lastblock(self):
        last = list(mycol.find({}, {'_id': 0}).sort([("timestamp", -1)]).limit(1))
        print(last)
        if not last:
            last_block = blockchain.create_block(0,00)
        else:
            last_block = last[0]
        return last_block
...
def mine():
    ...
    # mydict = {'block_number': block['block_number'] ,
